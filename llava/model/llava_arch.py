...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        if self.get_model().method == "VTC-CLS":
            token_num = self.get_model().token_num
            cls_attn, image_features = self.get_model().get_vision_tower()(images, method=self.get_model().method, dataset_name=dataset_name, start_layer=self.get_model().layer, token_num=token_num,)
            _, idx = torch.topk(cls_attn, token_num, dim=0, largest=True)  # [B, left_tokens] , sorted=True
            idx = idx.squeeze(0)
            old_position_ids = torch.arange(0, 576, device=image_features.device).unsqueeze(0)
            image_features = image_features[:, idx, :].clone()
            new_position_ids = old_position_ids[:, idx].clone()
            sorted_indices = torch.argsort(new_position_ids, dim=1)
            image_features = image_features[:, sorted_indices[0], :]
            new_position_ids = new_position_ids[:, sorted_indices[0]]
        elif "llava_prumerge" in self.get_model().method:
            token_num = self.get_model().token_num
            image_features = self.get_model().get_vision_tower()(images, method=self.get_model().method, dataset_name=dataset_name, start_layer=self.get_model().layer, token_num=token_num,)
        else:
